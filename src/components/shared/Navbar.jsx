    ...
    <nav className="w-full border-b border-gray-100 bg-white/80 backdrop-blur-md sticky top-0 z-[100]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 h-20 flex items-center justify-between gap-4 md:gap-8">
        ...
        <div className="flex items-center gap-4 lg:hidden">
          ...
          <button
            className="relative w-10 h-10 bg-white border border-gray-200 rounded-full flex flex-col items-center justify-center gap-[4.5px] z-50 overflow-hidden shadow-sm hover:bg-gray-50 transition-colors"
            onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)}
          >
            <span
              className={`block w-[18px] h-[2px] bg-slate-800 rounded-full transition-all duration-300 ease-out ${isMobileMenuOpen ? "transform rotate-45 translate-y-[6.5px]" : ""}`}
            />
            <span
              className={`block w-[18px] h-[2px] bg-slate-800 rounded-full transition-all duration-300 ease-out ${isMobileMenuOpen ? "opacity-0 translate-x-4" : "opacity-100"}`}
            />
            <span
              className={`block w-[18px] h-[2px] bg-slate-800 rounded-full transition-all duration-300 ease-out ${isMobileMenuOpen ? "transform -rotate-45 -translate-y-[6.5px]" : ""}`}
            />
