    ...
    <nav className="w-full border-b border-gray-100 bg-white/80 backdrop-blur-md sticky top-0 z-50">
        ...
        <div className="hidden lg:flex items-center gap-6 flex-1 justify-center">
          ...
          >
            <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
              <Search
                className={`h-4 w-4 transition-colors duration-300 ${
                  isSearchFocused ? "text-blue-600" : "text-slate-700"
                }`}
                strokeWidth={2.5}
              />
            </div>
            ...
            <div className="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
              <span
                className={`text-[10px] font-bold border rounded px-1.5 py-0.5 transition-colors duration-300 ${isSearchFocused ? "border-blue-200 text-blue-500" : "border-gray-200 text-gray-400"}`}
              >
                ⌘K
              </span>
            </div>
      ...
      {isMobileMenuOpen && (
        <div className="lg:hidden absolute top-20 left-0 w-full bg-white border-b border-gray-100 shadow-xl py-6 px-4 flex flex-col gap-4">
          ...
          <Link
            href="/jobs"
            className="text-base font-medium text-slate-600 p-3 hover:bg-gray-50 rounded-xl"
          ...
          <Link
            href="/bookmarks"
            className="flex items-center justify-between text-base font-medium text-slate-600 p-3 hover:bg-gray-50 rounded-xl"
            onClick={() => setIsMobileMenuOpen(false)}
          >
