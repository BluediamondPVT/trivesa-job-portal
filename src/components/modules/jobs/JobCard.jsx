...
export default function JobCard({ job, isMounted }) {
  ...
  const handleBookmarkClick = (e) => {
    e.stopPropagation(); // Card navigation ko block karne ke liye
    ...
    >
      ...
      <button
        ...
        className={`absolute top-4 right-4 sm:hidden p-2 rounded-full transition-all z-10 ${
          saved ? "bg-blue-50 text-blue-600" : "bg-gray-50 text-gray-400 hover:text-blue-600"
        }`}
      >
      ...
      <div className={`w-12 h-12 sm:w-16 sm:h-16 rounded-xl flex items-center justify-center font-bold text-xl sm:text-2xl text-white shrink-0 shadow-sm ${job.logoColor}`}>
        {job.logoText}
      </div>
      ...
      <div className="flex-1 w-full">
        ...
        {/* Title & Company */}
        <div className="pr-10 sm:pr-0"> {/* pr-10 on mobile so text doesn't hit the bookmark icon */}
        ...
        <div className="flex flex-wrap items-center gap-y-2 gap-x-4 sm:gap-x-5 text-[12px] sm:text-[13px] text-gray-500 font-medium mb-3 sm:mb-4">
          <span className="flex items-center gap-1.5">
            ...
            1-5 Yrs {/* Optional: Agar actual exp data ho toh yahan variable daal dena */}
          </span>
          <span className="flex items-center gap-1.5">
